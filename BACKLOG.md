...
*Note:* a live end-to-end network test was **deliberately not added** — the current
pandapipes (0.13) does not converge the repo's own `examples/06_*` minimal nets
(stale "needs fixes due to updated functions"), so such a test would be brittle. Fix
the examples first, then a network characterization test becomes worthwhile (would
also unlock C1/C3 and the ruff long-tail in `net_simulation_pandapipes`).
