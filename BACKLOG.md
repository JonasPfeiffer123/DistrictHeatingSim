...
context) and validates `np_results` before post-processing. *Still open:* the same
guard for `simplified_time_series_net`, explicit convergence diagnostics from the
solver, and detecting disconnected networks at build time (`create_network`).
