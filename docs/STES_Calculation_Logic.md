...
- [`SimpleThermalStorage_Calculation_Logic.md`](SimpleThermalStorage_Calculation_Logic.md) - Basic energy balance, heat losses, geometry
- [`StratifiedThermalStorage_Calculation_Logic.md`](StratifiedThermalStorage_Calculation_Logic.md) - Stratification, layer calculations, inter-layer conduction
...
Given heat input $\dot{Q}_{\text{in}}$ [kW] and temperatures:
...
Given heat output $\dot{Q}_{\text{out}}$ [kW]:
...
For layer $i$ with incoming flow:
...
Where:
- $\dot{m}_{\text{flow}}$ = Flow mass per hour [kg] = $\dot{m}$ [kg/s] × 3600 [s/h]
- $m_{\text{layer}}$ = Layer mass [kg] = $\rho \cdot V_{\text{layer}}$
- $c_p$ cancels out in the fraction

**Note:** This is equivalent to energy balance but computationally simpler (no need for Kelvin conversion or explicit $c_p$ multiplication).
...
**Typical value:** $T_{\text{max,rücklauf}} = 70°C$

**If violated:**
- Charging stopped: $\dot{m}_{\text{in}} = 0$, $\dot{Q}_{\text{in}} = 0$
- Excess heat tracked: $E_{\text{excess}} \mathrel{+}= \dot{Q}_{\text{in,available}} \cdot \Delta t$
...
**Typical value:** $\Delta T_{\min} = 10^{-3}$ K (numerical threshold)
...
Where:
- $T_{\text{supply,required}}$ = Required supply temperature from generators [°C]
- $\Delta T_{\text{VLT}}$ = Supply temperature tolerance [K] (typical: 15 K)

**Rationale:** Ensure adequate supply temperature quality for consumers.

**If violated:**
- Discharging stopped: $\dot{m}_{\text{out}} = 0$, $\dot{Q}_{\text{out}} = 0$
- Unmet demand tracked: $E_{\text{unmet}} \mathrel{+}= \dot{Q}_{\text{out,demand}} \cdot \Delta t$
...
See [`StratifiedThermalStorage_Calculation_Logic.md`, Section 6.2](StratifiedThermalStorage_Calculation_Logic.md#62-main-simulation-loop) for:
...
- Storage efficiency: $\eta = 1 - \sum Q_{\text{loss}} / \sum Q_{\text{in}}$
- Thermal capacity: $E_{\text{capacity}} = \rho \cdot V \cdot c_p \cdot \Delta T_{\text{range}}$

**From StratifiedThermalStorage:**
- Stratification index: $\text{SI} = (T_{\text{top}} - T_{\text{bottom}}) / (T_{\max} - T_{\min})$
- Effective temperature: $T_{\text{eff}} = \sum (T_i \cdot V_i) / \sum V_i$
...
Where:
- $\dot{Q}_{\text{in,actual}}$ may be 0 if stagnation occurs
- $\dot{Q}_{\text{out,actual}}$ may be 0 if supply insufficient
...
Given $\dot{Q} = \dot{m} \cdot c_p \cdot \Delta T$, check:
...
See [`SimpleThermalStorage_Calculation_Logic.md`, Section 9](SimpleThermalStorage_Calculation_Logic.md#9-model-limitations-and-assumptions) and [`StratifiedThermalStorage_Calculation_Logic.md`, Section 12](StratifiedThermalStorage_Calculation_Logic.md#122-stratification-specific-limitations).
...
- [`SimpleThermalStorage_Calculation_Logic.md`](SimpleThermalStorage_Calculation_Logic.md) - Base model fundamentals
- [`StratifiedThermalStorage_Calculation_Logic.md`](StratifiedThermalStorage_Calculation_Logic.md) - Stratification physics and layer calculations
...
|--------|-------------|------|
| $\dot{m}_{\text{in}}$ | Charging mass flow rate | kg/s |
| $\dot{m}_{\text{out}}$ | Discharging mass flow rate | kg/s |
| $T_{\text{supply}}$ | Generator supply temperature | °C |
| $T_{\text{return}}$ | Consumer return temperature | °C |
| $T_{\text{max,rücklauf}}$ | Maximum allowable return temperature to generators | °C |
| $\Delta T_{\text{VLT}}$ | Supply temperature tolerance | K |
| $E_{\text{excess}}$ | Total excess heat (stagnation) | kWh |
| $E_{\text{unmet}}$ | Total unmet demand | kWh |
| $t_{\text{stagnation}}$ | Stagnation duration | hours |
