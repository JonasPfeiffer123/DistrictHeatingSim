...
Where:
- :math:`h_{\text{eff}}` = Effective heat transfer coefficient at storage boundary [W/(m²·K)]
- :math:`L_c` = Characteristic length = :math:`V / A_{\text{surface}}` [m]
- :math:`k_{\text{medium}}` = Thermal conductivity of storage medium (water: ~0.6 W/(m·K))
...
Where:
- :math:`E` = Stored thermal energy [J] or [kWh]
- :math:`\dot{Q}_{\text{in}}` = Heat input power [kW]
- :math:`\dot{Q}_{\text{out}}` = Heat output power [kW]
- :math:`\dot{Q}_{\text{loss}}` = Heat loss power to environment [kW]
...
**Timestep:** :math:`\Delta t = 1` hour (hourly resolution)

**Unit consistency for** :math:`\Delta t = 1` **h:**
.. math::
...
Numerically: :math:`[\text{kW}] \times [1\,\text{h}] = [\text{kWh}]` (power integrated over 1 hour equals energy)
...
For typical STES systems with large thermal mass and good insulation, :math:`\Delta t = 1` hour is well within the stability limit.
...
At :math:`t=0`, no time interval has elapsed, so no heat losses are applied. The storage is initialized with its specified initial temperature and corresponding energy content.
...
Where:
- :math:`m = \rho \cdot V` = Storage mass [kg]
- :math:`c_p` = Specific heat capacity of storage medium [J/(kg·K)]
- :math:`T` = Storage temperature [°C]
- :math:`T_{\text{ref}}` = Reference temperature [°C] (typically 0°C)
...
- Conversion factor: :math:`1\,\text{kWh} = 3.6 \times 10^6\,\text{J}`
...
1. Temperature is capped at limit value: :math:`T = T_{\text{limit}}`
2. Energy is recalculated consistently: :math:`E = \rho \cdot V \cdot c_p \cdot (T_{\text{limit}} - T_{\text{ref}})`

**Physical interpretation:**
- :math:`T > T_{\max}`: Boiling, pressure relief, or charging stops
- :math:`T < T_{\min}`: Freezing protection or heating activation
...
Where:
- :math:`\Delta T` = Temperature difference between storage and environment [K]
- :math:`R_{\text{total}}` = Total thermal resistance [K/W]
- :math:`U = 1/(R \cdot A)` = Overall heat transfer coefficient [W/(m²·K)]
- :math:`A` = Heat transfer surface area [m²]
...
The actual insulation must satisfy: :math:`\delta_{\text{side}} > 2\delta_{\text{min}}`
...
Where :math:`L_{\text{char}}` is:
- Truncated cone: bottom radius :math:`r_{\text{bottom}}`
...
**Input dimensions:** :math:`(r, h)` - radius and height [m]
...
**Input dimensions:** :math:`(r_{\text{top}}, r_{\text{bottom}}, h)` [m]
...
**Input dimensions:** :math:`(l_{\text{top}}, w_{\text{top}}, l_{\text{bottom}}, w_{\text{bottom}}, h)` [m]
...
**Interpretation:**
- :math:`\eta = 1.0` (100%): No losses (ideal storage)
- :math:`\eta = 0.95` (95%): 5% energy lost to environment (typical for well-insulated STES)
- :math:`\eta < 0.80` (80%): Poor insulation or unfavorable geometry
...
- :math:`\rho = 1000` kg/m³
- :math:`c_p = 4186` J/(kg·K)
- :math:`V = 10,000` m³
- :math:`\Delta T = 70` K (25°C → 95°C)
...
**Time complexity:** :math:`O(n)` where :math:`n` = number of timesteps

**Space complexity:** :math:`O(n)` for storing time series results
...
| :math:`\text{Bi}` | Biot number | - |
| :math:`c_p` | Specific heat capacity | J/(kg·K) |
| :math:`E` | Stored thermal energy | J, kWh |
| :math:`h` | Height | m |
| :math:`h_{\text{eff}}` | Effective heat transfer coefficient | W/(m²·K) |
| :math:`K` | Thermal conductance coefficient | W/(m²·K) |
| :math:`k` | Thermal conductivity of medium | W/(m·K) |
| :math:`L_c` | Characteristic length | m |
| :math:`m` | Mass | kg |
| :math:`\dot{Q}` | Heat transfer rate (power) | W, kW |
| :math:`Q` | Heat energy | J, kWh |
| :math:`R` | Thermal resistance | K/W |
| :math:`r` | Radius | m |
| :math:`T` | Temperature | °C, K |
| :math:`t` | Time | h, s |
| :math:`U` | Overall heat transfer coefficient | W/(m²·K) |
| :math:`V` | Volume | m³ |
| :math:`\delta` | Insulation thickness | m |
| :math:`\Delta t` | Timestep | h |
| :math:`\eta` | Efficiency | - |
