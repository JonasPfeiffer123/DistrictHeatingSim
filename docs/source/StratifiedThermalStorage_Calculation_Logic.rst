...
.. note::
   ...
   ```SimpleThermalStorage_Calculation_Logic.md`` <SimpleThermalStorage_Calculation_Logic.md>`_
...
.. note::
   ...
   DOI: `10.1016/j.renene.2019.11.121 <https://doi.org/10.1016/j.renene.2019.11.121>`_
...
Each layer has:
- **Individual temperature** :math:`T_i` [°C]
- **Layer-specific volume** :math:`V_i` [m³] (varies for non-cylindrical geometries)
- **Thermal capacity** :math:`C_i = \rho \cdot V_i \cdot c_p` [J/K]
- **Independent heat loss** :math:`\dot{Q}_{\text{loss},i}` [kW]
...
**See ```SimpleThermalStorage_Calculation_Logic.md``, Section 6 <SimpleThermalStorage_Calculation_Logic.md#6-geometric-calculations>`_ for base geometric formulas.**
...
For layer :math:`i` (counting from top, :math:`i = 0, 1, ..., n-1`):
...
Similarly for :math:`a_{\text{bottom},i}` and :math:`b_{\text{bottom},i}` at :math:`\frac{i+1}{n}`.
...
**Variable interface area** at boundary between layers :math:`i` and :math:`i+1`:
...
- **Temperature**: :math:`T_i` drives heat transfer rate
- **Boundary conditions**: Air, soil, or no external contact

**See ```SimpleThermalStorage_Calculation_Logic.md``, Section 5 <SimpleThermalStorage_Calculation_Logic.md#5-heat-loss-calculations>`_ for detailed heat transfer formulas.**
...
Subsurface Layers (:math:`i = 1, ..., n-1`):
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
**Required:** :math:`\delta_{\text{side}} > 2\delta_{\min}`
...
- **Truncated Cone**: :math:`L_{\text{char}} = r_{\text{bottom}}`
- **Truncated Trapezoid**: :math:`L_{\text{char}} = \min(a_{\text{bottom}}, b_{\text{bottom}})`
...
Where:
- :math:`\lambda_{\text{medium}}` = Thermal conductivity of storage medium (water: ~0.6 W/(m·K))
- :math:`A_{\text{interface},i}` = Cross-sectional area between layers :math:`i` and :math:`i+1` [m²]
- :math:`\delta_{\text{layer}}` = Layer thickness [m]
- :math:`T_i - T_{i+1}` = Temperature difference [K]
...
**Energy transfer over timestep** :math:`\Delta t = 1` hour [kWh]:
...
**Upper layer** (loses heat if :math:`T_i > T_{i+1}`):
...
**Lower layer** (gains heat if :math:`T_i > T_{i+1}`):
...
- If :math:`T_i > T_{i+1}`: Heat flows **downward** (hot to cold)
- If :math:`T_i < T_{i+1}`: Heat flows **upward** (cold layer below hot - unstable, will naturally convect)
- If :math:`|T_i - T_{i+1}| < 10^{-6}`: Negligible gradient, skip calculation

**Physical interpretation:**
The model captures **conductive** heat transfer. In reality, if :math:`T_i < T_{i+1}` (cold over hot), **natural convection** would rapidly eliminate this unstable stratification. The charging/discharging logic is designed to prevent such unphysical states.
...
- Layer volumes: :math:`V_i` (geometry-dependent)
- Interface areas: :math:`A_{\text{interface},i}` (for conduction)
- Surface areas: :math:`A_{\text{top}}`, :math:`A_{\text{side}}`, :math:`A_{\text{bottom}}`
...
For each timestep :math:`t = 1, 2, ..., T`:
...
For each interface :math:`i = 0, 1, ..., n-2`:
...
**Case A: Discharging** (:math:`E_{\text{net}} < 0`)

Start from top layer (:math:`i = 0`) and extract heat downward:
...
For each layer :math:`i = 0, 1, ..., n-1` while :math:`E_{\text{needed}} > 10^{-6}`:
...
If :math:`E_{\text{needed}} \geq E_{\text{available},i}`:
.. math::
...
**Case B: Charging** (:math:`E_{\text{net}} > 0`)

Start from top layer (:math:`i = 0`) and add heat downward:
...
For each layer :math:`i = 0, 1, ..., n-1` while :math:`E_{\text{remaining}} > 10^{-6}`:
...
If :math:`E_{\text{remaining}} \geq E_{\text{capacity},i}`:
.. math::
...
The stratified model uses **explicit Euler method** with :math:`\Delta t = 1` hour, same as SimpleThermalStorage.

**See ```SimpleThermalStorage_Calculation_Logic.md``, Section 3 <SimpleThermalStorage_Calculation_Logic.md#3-numerical-integration-method>`_ for details on:**
- Stability criteria
- Unit consistency
- Timestep selection
...
1. Heat losses (based on :math:`T[t-1]`)
2. Inter-layer conduction (based on :math:`T` after losses)
3. Charging/discharging (based on :math:`T` after conduction)
...
- **Good accuracy**: For :math:`\Delta t = 1` hour and large thermal mass
...
Where :math:`\alpha = \lambda / (\rho c_p)` is thermal diffusivity.

**For water:** :math:`\alpha \approx 1.4 \times 10^{-7}` m²/s

**Typical layer thickness:** :math:`\delta_{\text{layer}} = 1-3` m

**Check for** :math:`\delta = 1` m, :math:`\Delta t = 3600` s:
...
|----------|---------|---------|
| Thermal capacity | :math:`C_i = \rho V_i c_p` | Avoid repeated multiplication |
| Conduction coefficient | :math:`k_i = \lambda A_i / \delta` | Avoid repeated division |
| Conversion factor | :math:`f = 3.6 \times 10^6` | Single definition |
...
|--------|--------|------------|---------------------|
| **Average temperature** | :math:`\bar{T}` | :math:`\frac{1}{n}\sum T_i` | < 2% |
| **Total stored energy** | :math:`E` | :math:`\sum E_i` | < 3% |
| **Heat losses** | :math:`Q_{\text{loss}}` | :math:`\sum Q_{\text{loss},i}` | < 5% |
| **Efficiency** | :math:`\eta` | :math:`\eta_{\text{strat}}` | Similar |
...
|---------|----------------------|--------------------------|
| **Temperature nodes** | 1 (uniform) | :math:`n` layers (5-50) |
...
| **Memory usage** | :math:`O(T)` | :math:`O(T \times n)` |
...
**Same as SimpleThermalStorage** - see `Section 7.1 <SimpleThermalStorage_Calculation_Logic.md#71-storage-efficiency>`_:
...
- **Sharp thermocline**: :math:`\delta \to 0` (ideal stratification)
- **Diffuse thermocline**: :math:`\delta \to H` (poor stratification)
...
**See ```SimpleThermalStorage_Calculation_Logic.md``, Section 9 <SimpleThermalStorage_Calculation_Logic.md#9-model-limitations-and-assumptions>`_ for:**
...
- Radius: :math:`r = 15` m
- Height: :math:`H = 20` m
- Volume: :math:`V = \pi \times 15^2 \times 20 = 14,137` m³

**Discretization:**
- Number of layers: :math:`n = 10`
- Layer thickness: :math:`\delta = 20/10 = 2` m
- Layer volume: :math:`V_{\text{layer}} = 1,414` m³ (uniform)

**Properties:**
- Medium: Water (:math:`\rho = 1000` kg/m³, :math:`c_p = 4186` J/(kg·K), :math:`\lambda = 0.6` W/(m·K))
- Temperature range: :math:`T_{\min} = 10°C`, :math:`T_{\max} = 90°C`
- Initial: Uniform at :math:`T_0 = 50°C`

**Insulation:**
- Top: :math:`\delta_{\text{top}} = 0.5` m, :math:`\lambda_{\text{top}} = 0.04` W/(m·K)
- Side: :math:`\delta_{\text{side}} = 0.4` m, :math:`\lambda_{\text{side}} = 0.04` W/(m·K)
- Soil: :math:`\lambda_{\text{soil}} = 1.5` W/(m·K), :math:`T_{\text{soil}} = 10°C`
...
- :math:`Q_{\text{in}} = 1000` kW  
- :math:`Q_{\text{out}} = 0` kW  
- :math:`E_{\text{net}} = 1000` kWh
...
Since :math:`1000 < 65,778`, **only the top layer is heated:**
...
Where :math:`\epsilon_{\text{conv}} \approx 0.1-0.3` for typical conditions.
...
- ```SimpleThermalStorage_Calculation_Logic.md`` <SimpleThermalStorage_Calculation_Logic.md>`_ - Base model documentation
- ```ThermalStorage`` class documentation <../src/districtheatingsim/heat_generators/simple_thermal_storage.py>`_ - Implementation details
...
|--------|-------------|------|
| :math:`A_{\text{interface},i}` | Cross-sectional area between layers :math:`i` and :math:`i+1` | m² |
| :math:`C_i` | Thermal capacity of layer :math:`i` | J/K |
| :math:`\delta_{\text{layer}}` | Thickness of each layer | m |
| :math:`\delta_{\text{thermocline}}` | Thermocline thickness | m |
| :math:`E_i` | Stored energy in layer :math:`i` | kWh |
| :math:`i` | Layer index (0 = top, n-1 = bottom) | - |
| :math:`k_{\text{cond},i}` | Conduction coefficient for interface :math:`i` | W/K |
| :math:`n` | Number of layers | - |
| :math:`\dot{Q}_{\text{cond},i}` | Conduction heat transfer rate between layers | W |
| :math:`\dot{Q}_{\text{loss},i}` | Heat loss from layer :math:`i` | kW |
| :math:`\text{SI}` | Stratification index | - |
| :math:`T_i` | Temperature of layer :math:`i` | °C |
| :math:`T_{\text{eff}}` | Effective temperature (volume-weighted) | °C |
| :math:`V_i` | Volume of layer :math:`i` | m³ |
...
|-------------|-------------|
| :math:`(1)` | After heat loss step |
| :math:`(2)` | After inter-layer conduction step |
| :math:`(3)` | After charging/discharging step |
...
- Heat losses: :math:`O(n)`
- Inter-layer conduction: :math:`O(n)`
- Charging/discharging: :math:`O(n)` worst case, :math:`O(1)` best case with early termination
- **Total:** :math:`O(n)` per timestep

**For annual simulation:**
- Timesteps: :math:`T = 8760`
- Layers: :math:`n = 10`
- **Total operations:** :math:`\approx 26,000` per simulation
...
**Storage arrays:**
- Temperature: :math:`T \times n` floats
- Energy, losses: :math:`T` floats
- Total: :math:`(T \times n + 2T) \times 8` bytes

**For T = 8760, n = 10:**
- Memory: :math:`(87,600 + 17,520) \times 8 = 840` KB
...
- **Next Level:** ```STES_Calculation_Logic.md`` <STES_Calculation_Logic.md>`_ - Mass flow-based STES with hydraulic constraints
- **Previous Level:** ```SimpleThermalStorage_Calculation_Logic.md` <SimpleThermalStorage_Calculation_Logic.md>`_ - Lumped capacitance base model
