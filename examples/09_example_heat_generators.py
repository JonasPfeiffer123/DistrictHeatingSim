...
def test_gas_boiler(Last_L, duration, Gaspreis, q, r, T, BEW, stundensatz, gBoiler=gas_boiler.GasBoiler(name="Gas_Boiler_1", spez_Investitionskosten=30, Nutzungsgrad=0.9, Faktor_Dimensionierung=1)):
    gBoiler.simulate_operation(Last_L, duration)
    gBoiler.calculate_heat_generation_cost(Gaspreis, q, r, T, BEW, stundensatz)
...
def test_power_to_heat(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, PTH=power_to_heat.PowerToHeat(name="PowerToHeat", spez_Investitionskosten=30, Nutzungsgrad=0.9, Faktor_Dimensionierung=1)):
    PTH.simulate_operation(Last_L, duration)
    PTH.calculate_heat_generation_cost(Strompreis, q, r, T, BEW, stundensatz)
...
def test_biomass_boiler(Last_L, duration, Holzpreis, q, r, T, BEW, stundensatz, bBoiler=biomass_boiler.BiomassBoiler(name="Biomasss_Boiler_1", P_BMK=200, Größe_Holzlager=40, spez_Investitionskosten=200, spez_Investitionskosten_Holzlager=400, 
    ...
    bBoiler.calculate_heat_generation_costs(Wärmemenge, Brennstoffbedarf, Holzpreis, q, r, T, BEW, stundensatz)
...
def test_biomass_boiler_storage(Last_L, duration, Holzpreis, q, r, T, BEW, stundensatz, bBoiler=biomass_boiler.BiomassBoiler(name="Biomasss_Boiler_1", P_BMK=200, Größe_Holzlager=40, spez_Investitionskosten=200, spez_Investitionskosten_Holzlager=400, 
...
def test_chp(Last_L, duration, Strompreis, Gaspreis, Holzpreis, q, r, T, BEW, stundensatz, chp=chp.CHP(name="BHKW", th_Leistung_BHKW=100, spez_Investitionskosten_GBHKW=1500, spez_Investitionskosten_HBHKW=1850, el_Wirkungsgrad=0.33, KWK_Wirkungsgrad=0.9, 
    ...
    chp.calculate_heat_generation_costs(chp.Wärmemenge_BHKW, chp.Strommenge_BHKW, chp.Brennstoffbedarf_BHKW, Brennstoffpreis, Strompreis, q, r, T, BEW, stundensatz)
...
def test_chp_storage(Last_L, duration, Strompreis, Gaspreis, Holzpreis, q, r, T, BEW, stundensatz, chp=chp.CHP(name="BHKW", th_Leistung_BHKW=100, spez_Investitionskosten_GBHKW=1500, 
                                                                                                               spez_Investitionskosten_HBHKW=1850, el_Wirkungsgrad=0.33, KWK_Wirkungsgrad=0.9, 
                                                                                                               min_Teillast=0.7, speicher_aktiv=True, Speicher_Volumen_BHKW=20, T_vorlauf=90, 
                                                                                                               T_ruecklauf=60, initial_fill=0.0, min_fill=0.2, max_fill=0.8, spez_Investitionskosten_Speicher=750, 
                                                                                                               BHKW_an=True, opt_BHKW_min=0, opt_BHKW_max=1000, opt_BHKW_Speicher_min=0, opt_BHKW_Speicher_max=100)):
    if chp.name == "BHKW":
        Brennstoffpreis = Gaspreis
    elif chp.name == "Holzgas-BHKW":
        Brennstoffpreis = Holzpreis
    ...
    chp.calculate_heat_generation_costs(chp.Wärmemenge_BHKW_Speicher, chp.Strommenge_BHKW_Speicher, chp.Brennstoffbedarf_BHKW_Speicher, Brennstoffpreis, Strompreis, q, r, T, BEW, stundensatz)
    chp.calculate_environmental_impact(Wärmemenge, Strommenge, Brennstoffbedarf)
...
def test_waste_heat_pump(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, VLT_L, COP_data, wasteHeatPump=heat_pumps.WasteHeatPump(name="Abwärme", Kühlleistung_Abwärme=50, Temperatur_Abwärme=30, spez_Investitionskosten_Abwärme=500, spezifische_Investitionskosten_WP=1000, min_Teillast=0.2)):

    wasteHeatPump.calculate_operation(Last_L, VLT_L, COP_data, duration)
    WGK = wasteHeatPump.calculate_heat_generation_costs(wasteHeatPump.max_Wärmeleistung, wasteHeatPump.Wärmemenge, wasteHeatPump.Strombedarf, wasteHeatPump.spez_Investitionskosten_Abwärme, Strompreis, q, r, T, BEW, stundensatz)
...
def test_river_heat_pump(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, VLT_L, COP_data, riverHeatPump=heat_pumps.RiverHeatPump(name="Flusswasser", Wärmeleistung_FW_WP=200, Temperatur_FW_WP=10, dT=0, spez_Investitionskosten_Flusswasser=1000, spezifische_Investitionskosten_WP=1000, min_Teillast=0.2)):   
    
    riverHeatPump.calculate_operation(Last_L, VLT_L, COP_data, duration)
    WGK = riverHeatPump.calculate_heat_generation_costs(riverHeatPump.Wärmeleistung_FW_WP, riverHeatPump.Wärmemenge, riverHeatPump.Strombedarf, riverHeatPump.spez_Investitionskosten_Flusswasser, Strompreis, q, r, T, BEW, stundensatz)
...
def test_geothermal_heat_pump(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, VLT_L, COP_data, geothermalHeatPump=heat_pumps.Geothermal(name="Geothermie", Fläche=200, Bohrtiefe=100, Temperatur_Geothermie=10, spez_Bohrkosten=100, spez_Entzugsleistung=50, Vollbenutzungsstunden=2400, Abstand_Sonden=10, spezifische_Investitionskosten_WP=1000, min_Teillast=0.2)):
    ...
    WGK = geothermalHeatPump.calculate_heat_generation_costs(geothermalHeatPump.max_Wärmeleistung, geothermalHeatPump.Wärmemenge, geothermalHeatPump.Strombedarf, geothermalHeatPump.spez_Investitionskosten_Erdsonden, Strompreis, q, r, T, BEW, stundensatz)
...
def test_aqva_heat(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, VLT_L, COP_data, aqva_heat=heat_pumps.AqvaHeat(name="AqvaHeat", nominal_power=100, temperature_difference=0)):
    # not implemented yet
    ...
    WGK = aqva_heat.calculate_heat_generation_costs(aqva_heat.Wärmeleistung, Wärmemenge, Strombedarf, aqva_heat.spez_Investitionskosten, Strompreis, q, r, T, BEW, stundensatz)

def test_solar_thermal(Last_L, duration, q, r, T, BEW, stundensatz, VLT_L, RLT_L, COP_data, TRY_data, solarThermal=solar_thermal.SolarThermal(name="STA", bruttofläche_STA=200, vs=20, Typ="Vakuumröhrenkollektor", 
    ...
    solarThermal.Wärmemenge, solarThermal.Wärmeleistung_kW, solarThermal.Speicherladung, solarThermal.Speicherfüllstand = solar_thermal.Berechnung_STA(solarThermal.bruttofläche_STA, solarThermal.vs, solarThermal.Typ, Last_L, VLT_L, RLT_L, 
                                                                                                        TRY_data, time_steps, calc1, calc2, duration, solarThermal.Tsmax, solarThermal.Longitude, solarThermal.STD_Longitude, 
    ...
    solarThermal.WGK = solarThermal.calculate_heat_generation_costs(q, r, T, BEW, stundensatz)
...
if __name__ == "__main__":
    ...
    test_power_to_heat(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, PTH)
    ...
    bBoiler = biomass_boiler.BiomassBoiler(name="Biomasss_Boiler_1", P_BMK=200, Größe_Holzlager=40, spez_Investitionskosten=200, 
                                           ...
                                           opt_Speicher_max=100)
    test_biomass_boiler(Last_L, duration, Holzpreis, q, r, T, BEW, stundensatz, bBoiler)
    ...
    bBoiler_storage = biomass_boiler.BiomassBoiler(name="Biomasss_Boiler_1", P_BMK=200, Größe_Holzlager=40, spez_Investitionskosten=200, 
                                                   ...
                                                   spez_Investitionskosten_Speicher=750, BMK_an=True, opt_BMK_min=0, opt_BMK_max=1000, opt_Speicher_min=0, 
                                                   opt_Speicher_max=100)
    test_biomass_boiler_storage(Last_L, duration, Holzpreis, q, r, T, BEW, stundensatz, bBoiler_storage)
    ...
    test_chp(Last_L, duration, Strompreis, Gaspreis, Holzpreis, q, r, T, BEW, stundensatz, CHP)
    ...
    riverHeatPump = heat_pumps.RiverHeatPump(name="Flusswasser", Wärmeleistung_FW_WP=200, Temperatur_FW_WP=10, dT=0, spez_Investitionskosten_Flusswasser=1000, 
                                             spezifische_Investitionskosten_WP=1000, min_Teillast=0.2)
    test_river_heat_pump(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, VLT_L, COP_data, riverHeatPump) 
    ...
    test_geothermal_heat_pump(Last_L, duration, Strompreis, q, r, T, BEW, stundensatz, VLT_L, COP_data, geothermal_heat_pump)
    ...
    test_solar_thermal(Last_L, duration, q, r, T, BEW, stundensatz, VLT_L, RLT_L, COP_data, TRY_data, solarThermal)
