...
class ProjectExplorer(QWidget):
    ...
    def __init__(self, folder_manager, config_manager, parent=None):
        ...
        if hasattr(self.folder_manager, "project_data_path") and self.folder_manager.project_data_path:
            ...
            self.set_base_path(
                os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "project_data")
            )
    ...
    def initUI(self):
        ...
        header_label.setStyleSheet(
            "color: #fff; background: #222; padding: 8px 12px; border-radius: 6px; margin-bottom: 4px;"
        )
        ...
        for i in range(self.project_tree.topLevelItemCount()):
            ...
            for j in range(variant_item.childCount()):
                ...
                if config_item.checkState(0) == Qt.CheckState.Checked:
                    self.selected_variants.append(config_item.data(0, Qt.ItemDataRole.UserRole))
...
class ComparisonDashboard(QWidget):
    ...
    def update_energy_chart(self):
        ...
        for i, variant in enumerate(self.variant_data):
            ...
            if techs and anteile:
                filtered_data = [
                    (tech, anteil, color)
                    for tech, anteil, color in zip(techs, anteile, colors, strict=False)
                    if anteil > 1.0
                ]
                if filtered_data:
                ...
                wedges, texts = ax.pie(
                    anteile_filtered, labels=None, colors=colors_filtered, autopct=None, startangle=90
                )
                legend_labels = [
                    f"{tech}: {anteil:.1f}%" for tech, anteil in zip(techs_filtered, anteile_filtered, strict=False)
                ]
    ...
    def clear_dashboard(self):
        ...
        # Clear charts
        for figure in [self.cost_figure, self.energy_figure, self.co2_figure, self.pe_figure, self.network_figure]:
            figure.clear()

        for canvas in [self.cost_canvas, self.energy_canvas, self.co2_canvas, self.pe_canvas, self.network_canvas]:
            canvas.draw()
    ...
    def load_variant_data(self, selected_variants):
        ...
        for item in selected_variants:
            try:
                ...
                QMessageBox.warning(
                    self, "Ladefehler", f"Fehler beim Laden von '{item.get('display_name', '?')}':\n{str(e)}"
                )
