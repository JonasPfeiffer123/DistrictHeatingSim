                        ...
                        try:
                            ...
                            if location and location.raw.get("address"):
                                ...
                                self.progress_update.emit(
                                    i + 1, total_features, f"Gebäude {i + 1}/{total_features}: {adresse}, {stadt}"
                                )
                        except Exception:
                            self.progress_update.emit(
                                i + 1, total_features, f"Gebäude {i + 1}/{total_features}: Geocoding fehlgeschlagen"
                            )
                    else:
                        self.progress_update.emit(
                            i + 1, total_features, f"Gebäude {i + 1}/{total_features}: Keine Koordinaten"
                        )
