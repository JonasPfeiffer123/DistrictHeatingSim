...
def validate_minimum_pressure_difference(
    net, target_dp_min_bar: float = 1.0, verbose: bool = True
...
def recalculate_net(net, mode: str = "bidirectional", iter: int = 100):
    ...
    try:
        ...
        raise RuntimeError(f"Network recalculation failed ({mode}, iter={iter}): {e}") from e
    return net


def COP_WP(
    VLT_L: float | np.ndarray, QT: float | np.ndarray, values: np.ndarray | None = None
...
def create_controllers(
    ...
    for i in range(len(net.heat_consumer)):
        ...
        ConstControl(
            ...
            variable="qext_w",
            element_index=i,
            data_source=placeholder_data_source_qext,
            profile_name=f"qext_w_{i}",
        )
        ...
        ConstControl(
            ...
            variable="treturn_k",
            element_index=i,
            data_source=placeholder_data_source_treturn,
        ...
        if min_supply_temperature_heat_consumer is not None and np.any(
            ...
            T_controller = MinimumSupplyTemperatureController(
                net, heat_consumer_idx=i, min_supply_temperature=min_supply_temperature_heat_consumer[i]
            )
    ...
    placeholder_df_supply_temp = pd.DataFrame(
        {"supply_temperature": [supply_temperature_heat_generator + KELVIN_OFFSET]}
    )
...
def init_diameter_types(
    net, v_max_pipe: float = 1.0, material_filter: str = "P235GH/PUR/PEHD", k: float = 0.1
) -> pp.pandapipesNet:
    """
...
def build_diameter_ladders(filtered_by_material) -> dict[str, list[str]]:
    ...
    for type_name, row in filtered_by_material.iterrows():
        ladders.setdefault(_insulation_grade(type_name), []).append((row["inner_diameter_mm"], type_name))
...
def optimize_diameter_types(
    net, v_max: float = 1.0, material_filter: str = "P235GH/PUR/PEHD", k: float = 0.1
) -> pp.pandapipesNet:
    """
...
def export_net_geojson(net, filename: str, crs: str = "EPSG:25833") -> dict:
    ...
    if hasattr(net, "heat_consumer") and not net.heat_consumer.empty:
        for idx, consumer in net.heat_consumer.iterrows():
            ...
            geometry = LineString([(from_junction["x"], from_junction["y"]), (to_junction["x"], to_junction["y"])])

            building_features.append(
                {"geometry": geometry, "connection_id": f"hast_{idx:03d}", "heat_demand_W": consumer.get("qext_w", 0)}
            )
    ...
    if hasattr(net, "circ_pump_pressure") and not net.circ_pump_pressure.empty:
        for idx, pump in net.circ_pump_pressure.iterrows():
            ...
            geometry = LineString(
                [(return_junction["x"], return_junction["y"]), (flow_junction["x"], flow_junction["y"])]
            )

            generator_features.append({"geometry": geometry, "generator_id": f"gen_{idx:03d}"})
