...
def create_pressure_plot(net, ax: plt.Axes | None = None, show_colorbar: bool = True):
    ...
    if not hasattr(net, "res_junction") or net.res_junction.empty:
        ax.text(0.5, 0.5, "Keine Simulationsergebnisse verfügbar", transform=ax.transAxes, ha="center", va="center")
        return ax
    ...
    try:
        ...
        if hasattr(net, "res_pipe") and not net.res_pipe.empty and hasattr(net, "junction_geodata"):
            for _idx, pipe in net.pipe.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    ax.plot([from_x, to_x], [from_y, to_y], color=pipe_color, linewidth=4, alpha=0.8, zorder=2)
        ...
        if hasattr(net, "heat_consumer") and len(net.heat_consumer) > 0 and hasattr(net, "junction_geodata"):
            for _idx, consumer in net.heat_consumer.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    mid_x = (from_x + to_x) / 2
                    mid_y = (from_y + to_y) / 2
                    ax.scatter(mid_x, mid_y, c="green", s=200, marker="s", edgecolors="black", linewidth=2, zorder=6)

        # Plot pumps
        if (
            hasattr(net, "circ_pump_const_pressure")
            and len(net.circ_pump_const_pressure) > 0
            and hasattr(net, "junction_geodata")
        ):
            for _idx, pump in net.circ_pump_const_pressure.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    mid_x = (from_x + to_x) / 2
                    mid_y = (from_y + to_y) / 2
                    ax.scatter(mid_x, mid_y, c="orange", s=200, marker="o", edgecolors="black", linewidth=2, zorder=6)
        ...
        ax.text(0.5, 0.5, f"Fehler beim Plotten: {str(e)}", transform=ax.transAxes, ha="center", va="center")
        ...
        if hasattr(net, "heat_consumer") and len(net.heat_consumer) > 0 and hasattr(net, "junction_geodata"):
            for _idx, consumer in net.heat_consumer.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    mid_x = (from_x + to_x) / 2
                    mid_y = (from_y + to_y) / 2
                    ax.scatter(mid_x, mid_y, c="blue", s=200, marker="s", edgecolors="white", linewidth=2, zorder=6)
        ...
        if (
            ...
            for _idx, pump in net.circ_pump_const_pressure.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    mid_x = (from_x + to_x) / 2
                    mid_y = (from_y + to_y) / 2
                    ax.scatter(mid_x, mid_y, c="green", s=200, marker="o", edgecolors="black", linewidth=2, zorder=6)
...
def create_velocity_plot(net, ax: plt.Axes | None = None):
    ...
    if not hasattr(net, "res_pipe") or net.res_pipe.empty:
        ax.text(0.5, 0.5, "Keine Rohrleitungsergebnisse verfügbar", transform=ax.transAxes, ha="center", va="center")
        return ax
        ...
        if "v_mean_m_per_s" in net.res_pipe.columns and hasattr(net, "junction_geodata"):
            ...
            for idx, pipe in net.pipe.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    ax.plot([from_x, to_x], [from_y, to_y], color=pipe_color, linewidth=line_width, alpha=0.8, zorder=2)
            ...
            if hasattr(net, "junction_geodata"):
                for _idx, pipe in net.pipe.iterrows():
                    ...
                    if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                        ...
                        to_x = net.junction_geodata.loc[to_junction, "x"]
                        to_y = net.junction_geodata.loc[to_junction, "y"]
                        ax.plot([from_x, to_x], [from_y, to_y], "purple", linewidth=3, alpha=0.7, zorder=1)
        ...
        if hasattr(net, "heat_consumer") and len(net.heat_consumer) > 0 and hasattr(net, "junction_geodata"):
            for _idx, consumer in net.heat_consumer.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    mid_x = (from_x + to_x) / 2
                    mid_y = (from_y + to_y) / 2
                    ax.scatter(mid_x, mid_y, c="blue", s=150, marker="s", edgecolors="white", linewidth=2, zorder=6)
        ...
        if (
            ...
            for _idx, pump in net.circ_pump_const_pressure.iterrows():
                ...
                if from_junction in net.junction_geodata.index and to_junction in net.junction_geodata.index:
                    ...
                    mid_x = (from_x + to_x) / 2
                    mid_y = (from_y + to_y) / 2
                    ax.scatter(mid_x, mid_y, c="orange", s=150, marker="o", edgecolors="black", linewidth=2, zorder=6)
...
def create_pressure_profile(net, ax: plt.Axes | None = None):
    ...
    try:
        ...
        ax.text(
            0.5, 0.5, f"Druckprofil nicht verfügbar\nFehler: {str(e)}", transform=ax.transAxes, ha="center", va="center"
        )
...
def create_comparison_dashboard(net, figsize=(16, 12)):
    ...
    try:
        pp_plot.simple_plot(
            net, ax=axes[0, 0], show_plot=False, junction_size=0.02, heat_consumer_size=0.1, pump_size=0.1
        )
        axes[0, 0].set_title("Netzwerk Topologie")
    except Exception:
        axes[0, 0].text(
            0.5, 0.5, "Netzwerk Topologie\nnicht verfügbar", transform=axes[0, 0].transAxes, ha="center", va="center"
        )
