...
def update_heat_consumer_temperature_controller(
    net, min_supply_temperature_heat_consumer: np.ndarray | list, time_steps: range, start: int, end: int
) -> None:
    """
    ...
    for ctrl in net.controller.object.values:
        if isinstance(ctrl, MinimumSupplyTemperatureController):
            ...
            df_return_temp = pd.DataFrame(index=time_steps, data={"min_supply_temperature": values})
            data_source_return_temp = DFData(df_return_temp)
            ctrl.data_source = data_source_return_temp
            controller_count += 1


def update_heat_consumer_return_temperature_controller(
    net, return_temperature_heat_consumer: np.ndarray | list, time_steps: range, start: int, end: int
) -> None:
    """
    ...
    for i, return_temp_profile in enumerate(return_temperature_heat_consumer):
        # Check if static or time-dependent
        if np.isscalar(return_temp_profile) or (
            isinstance(return_temp_profile, np.ndarray) and return_temp_profile.ndim == 0
        ):
            # Single value: repeat for all time steps
            values = np.full(len(time_steps), return_temp_profile + KELVIN_OFFSET)
        elif (
            isinstance(return_temp_profile, np.ndarray)
            and return_temp_profile.ndim == 1
            and len(return_temp_profile) == 1
        ):
        ...
        df_return_temp = pd.DataFrame(index=time_steps, data={f"treturn_k_{i}": values})
        data_source_return_temp = DFData(df_return_temp)
...
def update_secondary_producer_controller(
    net, secondary_producers: list[Any], time_steps: range, start: int, end: int
) -> None:
    """
...
def update_heat_generator_supply_temperature_controller(
    net, supply_temperature: np.ndarray, time_steps: range, start: int, end: int
) -> None:
    """
...
def time_series_preprocessing(NetworkGenerationData) -> Any:
    ...
    if NetworkGenerationData.supply_temperature_control == "Gleitend":
        ...
        slope = (
            ...
            NetworkGenerationData.min_air_temperature_heat_generator
            - NetworkGenerationData.max_air_temperature_heat_generator
        )
    ...
    if NetworkGenerationData.building_temperature_checked and NetworkGenerationData.netconfiguration != "kaltes Netz":
        NetworkGenerationData.min_supply_temperature_heat_consumer = (
            NetworkGenerationData.supply_temperature_buildings_curve + NetworkGenerationData.dT_RL
        )
        NetworkGenerationData.return_temperature_heat_consumer = (
            NetworkGenerationData.return_temperature_buildings_curve + NetworkGenerationData.dT_RL
        )
    ...
    ### if netconfiguration is "kaltes Netz", the heat demand and power consumption are calculated using the COP calculation
        ...
        NetworkGenerationData.min_supply_temperature_heat_consumer = (
            NetworkGenerationData.return_temperature_heat_consumer + NetworkGenerationData.dT_RL
        )
        NetworkGenerationData.return_temperature_heat_consumer = (
            NetworkGenerationData.return_temperature_buildings_curve + NetworkGenerationData.dT_RL
        )
        ...
        cop, _ = COP_WP(
            NetworkGenerationData.supply_temperature_buildings_curve,
            NetworkGenerationData.return_temperature_heat_consumer,
            COP_file_values,
        )
    ...
    ### if netconfiguration is not "kaltes Netz", the heat demand and power consumption are calculated using the COP calculation
        ...
        not NetworkGenerationData.building_temperature_checked
        and NetworkGenerationData.netconfiguration == "kaltes Netz"
    ):
        cop, _ = COP_WP(
    ...
    if NetworkGenerationData.secondary_producers:
        ...
        mass_flow = NetworkGenerationData.waerme_ges_kW / (
...
def thermohydraulic_time_series_net(NetworkGenerationData) -> Any:
    ...
    time_steps = range(
        0,
        len(
            NetworkGenerationData.waerme_hast_ges_W[0][
    ...
    if NetworkGenerationData.secondary_producers:
        update_secondary_producer_controller(
            NetworkGenerationData.net,
            NetworkGenerationData.secondary_producers,
            time_steps,
            NetworkGenerationData.start_time_step,
            NetworkGenerationData.end_time_step,
        )
    ...
    if (
        NetworkGenerationData.min_supply_temperature_heat_consumer is not None
...
def simplified_time_series_net(NetworkGenerationData) -> Any:
    ...
    design_results = {"Heizentrale Haupteinspeisung": {}, "weitere Einspeisung": {}}

    # Get design state from pressure pumps (main generators)
    if (
        hasattr(NetworkGenerationData.net, "res_circ_pump_pressure")
        and len(NetworkGenerationData.net.res_circ_pump_pressure) > 0
    ):
    ...
    if (
        hasattr(NetworkGenerationData.net, "res_circ_pump_mass")
        and len(NetworkGenerationData.net.res_circ_pump_mass) > 0
    ):
    ...
    total_generation_design = sum(
        [data["qext_kW_design"] for pump_type in design_results.values() for data in pump_type.values()]
    )
    ...
    building_demand_series = NetworkGenerationData.waerme_ges_kW[
        NetworkGenerationData.start_time_step : NetworkGenerationData.end_time_step
    ]
    ...
    for pump_type, pumps in design_results.items():
        for idx, design_data in pumps.items():
            ...
            share_of_generation = (
                design_data["qext_kW_design"] / total_generation_design if total_generation_design > 0 else 1.0
            )
...
def calculate_results(
    net, net_results: dict, cp_kJ_kgK: float = CP_WATER_KJ_KGK
    ...
    pump_results = {"Heizentrale Haupteinspeisung": {}, "weitere Einspeisung": {}}
    ...
    if "circ_pump_pressure" in net:
        for idx, _row in net.circ_pump_pressure.iterrows():
            pump_results["Heizentrale Haupteinspeisung"][idx] = {
                ...
                "deltap": net_results["res_circ_pump_pressure.p_to_bar"][:, idx]
                - net_results["res_circ_pump_pressure.p_from_bar"][:, idx],
                ...
                "qext_kW": net_results["res_circ_pump_pressure.mdot_from_kg_per_s"][:, idx]
                * cp_kJ_kgK
                * (
                    net_results["res_circ_pump_pressure.t_to_k"][:, idx]
                    - net_results["res_circ_pump_pressure.t_from_k"][:, idx]
                ),
            }
    ...
    if "circ_pump_mass" in net:
        for idx, _row in net.circ_pump_mass.iterrows():
            pump_results["weitere Einspeisung"][idx] = {
                ...
                "deltap": net_results["res_circ_pump_mass.p_to_bar"][:, idx]
                - net_results["res_circ_pump_mass.p_from_bar"][:, idx],
                ...
                "qext_kW": net_results["res_circ_pump_mass.mdot_from_kg_per_s"][:, idx]
                * cp_kJ_kgK
                * (
                    net_results["res_circ_pump_mass.t_to_k"][:, idx]
                    - net_results["res_circ_pump_mass.t_from_k"][:, idx]
                ),
            }
...
def save_results_csv(
    time_steps: np.ndarray, total_heat_KW: np.ndarray, strom_wp_kW: np.ndarray, pump_results: dict, filename: str
) -> None:
    """
