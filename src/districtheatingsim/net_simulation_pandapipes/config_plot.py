...
def config_plot(
    ...
    def make_annotation(
        ...
        line_points: list[tuple[float, float]] | None = None,
        visible: bool = False,
    ) -> dict[str, Any]:
        """
    ...
    if show_pump and hasattr(net, "circ_pump_pressure"):
        for pump in net.circ_pump_pressure.index:
            try:
                from_x, from_y = net.junction_geodata.loc[
                    net.circ_pump_pressure.at[pump, "return_junction"], ["x", "y"]
                ]
                ...
                if hasattr(net, "res_circ_pump_pressure"):
                    deltap = (
                        net.res_circ_pump_pressure.loc[pump, "p_to_bar"]
