...
class InteractiveNetworkPlot:
    ...
    def create_interactive_plot_with_controls(
        self, basemap_style: str = "carto-positron", colorscale: str = "Viridis"
    ) -> go.Figure:
        """
        ...
        for comp_type, params in available_params.items():
            for param in params:
                visualizations.append(
                    (f"{comp_type.replace('_', ' ').title()}: {self._get_parameter_label(param)}", comp_type, param)
                )
    ...
    def create_plot_for_parameter(
        self, component_type: str, parameter: str, basemap_style: str = "carto-positron", colorscale: str = "Viridis"
    ) -> go.Figure:
        """
    ...
    def create_plot(
        ...
        if show_pipes:
            self._add_pipes(parameter if component_type == "pipe" else None, colorscale, show=show_pipes)

        # Add heat consumers
        if show_heat_consumers:
            self._add_heat_consumers(
                parameter if component_type == "heat_consumer" else None, colorscale, show=show_heat_consumers
            )

        # Add pumps
        if show_pumps:
            self._add_pumps(parameter if component_type == "pump" else None, colorscale, show=show_pumps)

        # Add flow controls
        if show_flow_controls:
            self._add_flow_controls(
                parameter if component_type == "flow_control" else None, colorscale, show=show_flow_controls
            )

        # Add junctions (on top for better interaction)
        if show_junctions:
            self._add_junctions(parameter if component_type == "junction" else None, colorscale, show=show_junctions)
    ...
    def _add_junctions(self, parameter: str | None, colorscale: str, show: bool = True):
        ...
        if data.values is not None:
            marker = dict(
                ...
                showscale=True,
                colorbar=dict(
                    title=dict(text=self._get_parameter_label(parameter), side="right", font=dict(size=12)),
                    x=1.0,
                    xanchor="left",
                    thickness=15,
                    len=0.6,
    ...
    def _configure_layout(self, gdf_junctions: gpd.GeoDataFrame, basemap_style: str):
        ...
        self.fig.update_layout(
            ...
            height=600,
            hovermode="closest",
            modebar=dict(orientation="v", bgcolor="rgba(255, 255, 255, 0.8)", color="#2c3e50", activecolor="#3498db"),
        )
