...
class NetworkGeoJSONSchema:
    ...
    def split_to_legacy_format(
        geojson: dict[str, Any], crs: str = None
        ...
        building_gdf = (
            gpd.GeoDataFrame.from_features(building_features, crs=crs) if building_features else gpd.GeoDataFrame()
        )
        generator_gdf = (
            gpd.GeoDataFrame.from_features(generator_features, crs=crs) if generator_features else gpd.GeoDataFrame()
        )
