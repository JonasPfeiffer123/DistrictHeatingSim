...
def create_steiner_tree(
    street_graph: nx.MultiDiGraph, terminal_points: gpd.GeoDataFrame, weight: str = "length"
) -> nx.Graph:
    """
    ...
    for _idx, terminal in terminal_points.iterrows():
        nearest_node = ox.distance.nearest_nodes(street_graph, terminal.geometry.x, terminal.geometry.y)
        terminal_nodes.append(nearest_node)

    logger.debug(f"Mapped {len(terminal_nodes)} terminals to graph nodes")

    # Create Steiner Tree
    steiner_tree = nx.algorithms.approximation.steiner_tree(street_graph.to_undirected(), terminal_nodes, weight=weight)
...
def connect_terminals_with_edge_splitting(
    ...
    for u, v in steiner_tree.edges():
        ...
        edge_dict[(u, v)] = {"line": LineString([u_coords, v_coords]), "u_coords": u_coords, "v_coords": v_coords}
    ...
    for idx, terminal in terminal_points.iterrows():
        ...
        if best_type == "split":
            edges_to_split[best_edge].append({"split_coords": best_point, "terminal_idx": idx})
    ...
    logger.info(
        f"Connections: {len(connection_info)} total ({n_node_connections} to nodes, {n_split_connections} splits)"
    )
...
def remove_dead_ends(
    network_gdf: gpd.GeoDataFrame, protected_endpoints: set, max_iterations: int = 10
) -> gpd.GeoDataFrame:
    """
...
def create_connection_lines(connection_info: list[dict[str, Any]], crs: str = "EPSG:25833") -> gpd.GeoDataFrame:
    """
...
def create_return_network(
    supply_network: gpd.GeoDataFrame, offset_x: float = 1.0, offset_y: float = 0.0
) -> gpd.GeoDataFrame:
    """
    ...
    return_network["geometry"] = return_network.geometry.apply(lambda geom: offset_geometry(geom, offset_x, offset_y))
...
def create_hast_connections(
    buildings: gpd.GeoDataFrame, offset_x: float = 1.0, offset_y: float = 0.0, include_building_data: bool = True
) -> gpd.GeoDataFrame:
    """
    ...
    for idx, building in buildings.iterrows():
        ...
        hast_properties = {"geometry": hast_line, "building_id": idx, "length_m": hast_line.length}
...
def create_generator_connection(
    generator_coords: tuple[float, float], offset_x: float = 1.0, offset_y: float = 0.0, crs: str = "EPSG:25833"
) -> gpd.GeoDataFrame:
    """
    ...
    erzeuger_gdf = gpd.GeoDataFrame(
        [{"geometry": erzeuger_line, "generator_id": 0, "length_m": erzeuger_line.length}], crs=crs
    )
...
def generate_osmnx_network(
    ...
    logger.info(
        f"Total terminals: {len(buildings)} buildings + {len(generator_coords)} generator(s) = {len(all_terminals)}"
    )

    # Step 3: Create Steiner Tree
    logger.info("Step 3/12: Creating Steiner Tree")
    steiner_tree = create_steiner_tree(street_graph=street_graph, terminal_points=all_terminals, weight="length")
    ...
    main_network = build_network_from_split_edges(
        steiner_tree=steiner_tree, street_graph=street_graph, edges_to_split=edges_to_split, crs=target_crs
    )
    ...
    if remove_dead_ends_flag:
        ...
        main_network = remove_dead_ends(
            network_gdf=main_network, protected_endpoints=protected_endpoints, max_iterations=max_dead_end_iterations
        )

    # Step 6: Create connection lines
    logger.info("Step 6/12: Creating connection lines")
    connections = create_connection_lines(connection_info=connection_info, crs=target_crs)

    # Step 7: Combine supply network
    logger.info("Step 7/12: Combining supply network")
    supply_network = gpd.GeoDataFrame(pd.concat([main_network, connections], ignore_index=True), crs=target_crs)
    logger.info(
        f"Supply network: {len(supply_network)} segments ({len(main_network)} main + {len(connections)} connections)"
    )

    # Step 8: Create return network
    logger.info("Step 8/12: Creating return network")
    return_network = create_return_network(supply_network=supply_network, offset_x=return_offset, offset_y=0.0)

    # Step 9: Create HAST connections
    logger.info("Step 9/12: Creating HAST connections")
    hast_connections = create_hast_connections(
        buildings=buildings, offset_x=return_offset, offset_y=0.0, include_building_data=include_building_data
    )
    ...
    for idx, gen_coords in enumerate(generator_coords):
        gen_conn = create_generator_connection(
            generator_coords=gen_coords, offset_x=return_offset, offset_y=0.0, crs=target_crs
        )
        # Add generator index
        gen_conn["generator_id"] = idx
        generator_connections_list.append(gen_conn)

    generator_connection = gpd.GeoDataFrame(pd.concat(generator_connections_list, ignore_index=True), crs=target_crs)
...
def generate_and_export_osmnx_layers(
    ...
    try:
        ...
        buildings = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.UTM_X, df.UTM_Y), crs=target_crs)
